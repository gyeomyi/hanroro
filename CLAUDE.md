...
(앨범 페이지에서만 표지색으로 덮인다). 밝은 배경에서 대비가 안 나오므로 **원색을 글자색으로 쓰지 말 것**
(글자에는 `--grapefruit`/`--apricot`을 쓴다. 이 둘은 테마별로 대비를 맞춰둔 값이다).

### 눌리는 것의 채움 — `--btn-bg` / `--btn-fg` (2026-08-03 변경)
**버튼을 자몽→살구 그라디언트로 채우지 말 것.** 예전엔 `.btn` · 방명록 제출 · 켜진 칩 ·
모달 확인 · 재생 버튼이 전부 그 그라디언트였고, 페이지에서 그것들만 사탕색 덩어리로 튀어나왔다.
지금은 **본문 먹/종이를 뒤집어 채운다** — 대비는 가장 세면서 색은 페이지 안에 남는다.
```css
--btn-bg: var(--text);   /* 밤: 종이 → 낮: 먹. 테마를 따라 알아서 뒤집힌다 */
--btn-fg: var(--bg);
```
- `.btn` `.gb-form button` `.hr-swal-confirm` `.chip.is-on` `.gig-dday.is-today`가 이 둘을 쓴다
- **앨범 페이지는 `album.css`의 `:root[data-album]`이 이 둘을 그 앨범 브랜드색으로 덮는다**
  (`--btn-bg:var(--brand-1)` · `--btn-fg:var(--btn-ink)`). 거기선 색이 곧 세계라 남긴 것이고,
  **단색이다 — 그라디언트로 되돌리지 말 것.** 단색이라야 표지에서 뽑은 색이 그대로 읽힌다
- 여명 그라디언트는 **안 눌리는 자리**에만 남았다: 히어로 '로로'(`--hero-rr`) · 지평선(`.hero::after`) ·
  진행 막대 · `0+0=∞`(`.formula .inf`, `.egg-formula`) · 모달 상단 띠 · 표지 자리표시자.
  이건 정체성이라 그대로 둔다
- 앨범 타이틀 칩(`.trk-flag`)은 색은 남기되 **단색**이다. 그라디언트를 걷었다
- 대비 실측(전부 AA 통과): 밤 15.8 · 낮 12.8 · ep1 7.3 · ep2 5.0 · ep3 5.2 · single 5.6 · youandi 9.4

### 사진 위 조작부 — `--glass` / `--glass-ink`
재생 버튼(`.pl-play`)처럼 **사진 위에 얹히는** 것에만 쓴다. 두 테마 모두 어두운 사진 위라 값이 하나뿐이다.
- 예전엔 자몽→살구로 채운 **동그라미 + 분홍 후광**이었다. 이 페이지의 면은 전부 2px 각인데
  그 원 하나만 다른 사이트에서 옮겨 붙인 것처럼 떠 있었다
- 지금은 `.btn`과 같은 **2px 각**에 반투명 먹유리(`color-mix`로 62%) + 종이색 헤어라인 +
  `backdrop-filter:blur(6px)`. 마우스를 올리면 종이로 굳고 삼각형이 먹으로 뒤집힌다
- **원형으로 되돌리지 말 것.** 각이 페이지와 맞는 쪽이다
...
- ~~**떠 있는 장식 배지**(`.badge*`)~~ — **2026-08-03에 통째로 없앴다.** 히어로에 상 이름,
  프로필 사진에 'SINCE 2022'와 '0+0=∞', 링크 사진에 '3rd EP' 넉 장이 기울어진 채 둥둥 떠 있었다.
  넷 다 `aria-hidden`인 순수 장식인 데다 **같은 정보가 바로 옆 본문에 이미 있었고**
  (데뷔일은 `.about-meta`에, 수상은 자몽살구클럽 태그·행보·링크 카드에 세 번),
  사진 위에 스티커를 붙여 조잡해 보이게 하고 있었다. `@keyframes badge-float`까지 함께 제거.
  **되살리지 말 것.** `0+0=∞` 모티프는 자몽살구클럽 섹션의 큰 타이포(`.formula`)와
  숨은 문장·파비콘에 그대로 살아 있다
...
- **반응형**: 1100px(링크 그리드 2열+사진 배너), 900px(about 1단), 760px(헤더 세로·푸터 2단), 480px(버튼 full-width)
- **접근성**: 두 테마 모두 **WCAG AA 통과 확인됨**(본문/서브/캡션/강조 전부 ≥4.5:1). `:focus-visible` 아웃라인, `prefers-reduced-motion` 시 애니메이션·AOS 전부 정지
...
- **`--btn-ink`** — 브랜드색으로 채운 면 위의 글자색. ep1·ep2는 원색이 어두워
  ...
  (`.trk-flag`)와 `--btn-fg`로 넘어간 `.btn`·`.hr-swal-confirm`이 이 값을 쓴다
...
2. **프로필**: 사진(장식 배지 없음) + 약력 3단락 + `<dl>` 메타 4칸 + 하퍼스 바자 인터뷰 풀쿼트
...
- **배포**: `main`에 푸시하면 Vercel이 자동으로 올린다 (2026-08-03 라이브 대조 확인).
  반영이 안 보이면 브라우저 캐시부터 의심할 것 — `vercel.json`이 `css/js`에 `max-age=86400`을
  걸어둬서(파일명에 해시가 없다) 하루까지 옛 CSS가 남을 수 있다. `?cb=<난수>`를 붙여 볼 것
