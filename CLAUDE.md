...
- 여명 그라디언트는 **안 눌리는 자리**에만 남았다: 히어로 '로로'(`--hero-rr`) · 지평선(`.hero::after`) ·
  ...
  이건 정체성이라 그대로 둔다
- 배지(`.badge-solid`)와 앨범 타이틀 칩(`.trk-flag`)은 색은 남기되 **단색**이다.
  분홍 후광(`box-shadow`)도 걷었다 — 그라디언트와 후광 둘이 '사탕'처럼 보이게 하던 부분이다
...
- 예전엔 자몽→살구로 채운 **동그라미 + 분홍 후광**이었다. 이 페이지에서 둥근 건 배지 몇 개뿐이고
  나머지는 전부 2px 각진 면이라, 그 원 하나만 다른 사이트에서 옮겨 붙인 것처럼 떠 있었다
